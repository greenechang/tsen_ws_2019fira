...
void ActionModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  if(ros::param::get("/package_name",package_name)){
    path = ros::package::getPath(package_name) + "/data/"+"motion_4095.bin";
    ROS_INFO("Action module Got param");
  }
  
  else
    path = ros::package::getPath("op3_action_module") + "/data/"+"motion_4095.bin";
