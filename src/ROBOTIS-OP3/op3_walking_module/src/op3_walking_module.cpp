...
void WalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  if(ros::param::get("/package_name",package_name))
    default_param_path = ros::package::getPath(package_name) + "/config/param.yaml";
  
  else 
    default_param_path = ros::package::getPath("op3_walking_module") + "/config/param.yaml";
...
void WalkingModule::loadWalkingParam(const std::string &path)
{
  ...
  {
    ROS_ERROR("Walking: Fail to load yaml file.");
    return;
  }
